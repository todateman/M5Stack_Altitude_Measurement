...
- `include/AltitudeEKF.h`: 汎用 気圧->高度演算、2状態高度EKF（ヘッダオンリー）
- `platformio.ini`: PlatformIO設定

## ドキュメント

- [docs/AltitudeEKF_usage.md](docs/AltitudeEKF_usage.md): `AltitudeEKF.h` 利用ガイド（English）
- [docs/AltitudeEKF_usage_ja.md](docs/AltitudeEKF_usage_ja.md): `AltitudeEKF.h` 利用ガイド（日本語）
...
ここで `σ_a` は加速度ノイズの標準偏差（実装では `EKF_PROCESS_SIGMA_A`）です。
...
- `EKF_PROCESS_SIGMA_A`（プロセスノイズ、加速度ノイズ標準偏差）
- `R_GNSS`（GNSS観測ノイズ分散）
...
- 推定高度が加減速に対して鈍い → `EKF_PROCESS_SIGMA_A` を少し上げる
- 静止時に速度推定がふらつく → `EKF_PROCESS_SIGMA_A` を少し下げる
...
2. 次に上下移動テストで `EKF_PROCESS_SIGMA_A` を調整し、応答速度と安定性のバランスを取る
...
| 想定環境 | `R_BARO` | `EKF_PROCESS_SIGMA_A` | `R_GNSS` | ねらい |
| --- | ---: | ---: | ---: | --- |
